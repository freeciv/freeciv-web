...
echo "docker ALL = (root) NOPASSWD: /docker/scripts/docker-build*.sh, /docker/scripts/start-freeciv-web.sh, /usr/sbin/service nginx *, /usr/sbin/service tomcat8 *, /usr/sbin/service *, /etc/init.d/tomcat8 *, /bin/chmod, /bin/ls, /bin/cat, /usr/bin/mysql *  /usr/share/tomcat8/bin/catalina.sh *" >> /etc/sudoers
