...
# The CivCom handles communication between freeciv-proxy and the Freeciv C server.
class CivCom(Thread):
  ...
  def close_connection(self):
    if (logger.isEnabledFor(logging.INFO)):
      logger.info("Server connection closed. Removing civcom thread for " + self.username);
  ...
  def send_buffer_append(self, data):
    try:
      self.send_buffer.append(data.decode(encoding="utf-8", errors="ignore"));
    except UnicodeDecodeError:
      if (logger.isEnabledFor(logging.ERROR)):
        logger.error("Unable to decode string from civcom socket, for user: " + self.username);
      return;
  ...
  def send_packets_to_civserver(self):
    if (self.civserver_messages is None or self.socket is None): return;

    try:
      for net_message in self.civserver_messages:
        header = pack('>HH', len(net_message), 0);
        self.socket.sendall(header + net_message.encode('utf-8') + b'\0');
    except:
      self.send_error_to_client("Proxy unable to communicate with civserver on port " 
                                + str(self.civserverport));
    finally:
      self.civserver_messages = [];
